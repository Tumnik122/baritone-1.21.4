...
public abstract class AbstractNodeCostSearch implements IPathFinder, Helper {
    ...
    /**
     * Real-time count of nodes explored — exposed for HUD/stats display.
     * Updated from the pathfinding thread; read from render thread (volatile is sufficient).
     */
    public volatile int nodesExplored = 0;

    /**
     * Real-time movements-considered counter for stats.
     */
    public volatile int movementsConsidered = 0;
    ...
    AbstractNodeCostSearch(BetterBlockPos realStart, int startX, int startY, int startZ, Goal goal, CalculationContext context) {
        ...
        // Use at least 8192 initial map capacity to reduce rehashing during intensive search
        int mapSize = Math.max(8192, Baritone.settings().pathingMapDefaultSize.value);
