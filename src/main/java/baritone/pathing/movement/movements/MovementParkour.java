        ...
        if (ctx.playerFeet().equals(dest)) {
            ...
            if (ctx.playerFeet().equals(src.relative(direction)) || ctx.player().position().y - src.y > 0.0001) {
                ...
                if (dist == 3 && !ascend) { // this is a 2 block gap, dest = src + direction * 3
                    ...
                    // Add subtle jitter (±0.025) to the trigger threshold to prevent
                    // perfectly consistent jump timing, which flags GrimAC timer/patterns.
                    double jitter = Baritone.settings().humanizedRotations.value
                            ? (RANDOM.nextDouble() - 0.5) * 0.05
                            : 0.0;
                    if (distFromStart < 0.7 + jitter) {
                        return state;
                    }
                }
