...
public interface Helper {
    ...
    default void logNotification(String message, boolean error) {
        ...
        if (BaritoneAPI.getSettings().toastNotifications.value) {
            Component title = Component.literal("Baritone");
            Component subtitle = Component.literal(message);
            Minecraft.getInstance().execute(() -> BaritoneAPI.getSettings().toaster.value.accept(title, subtitle));
        }
        if (BaritoneAPI.getSettings().soundNotifications.value) {
            Minecraft.getInstance().execute(() -> {
                Minecraft mc = Minecraft.getInstance();
                if (mc.player != null) {
                    float pitch = error ? 0.7f : 1.2f;
                    net.minecraft.sounds.SoundEvent sound = error ? net.minecraft.sounds.SoundEvents.NOTE_BLOCK_BASS.value() : net.minecraft.sounds.SoundEvents.EXPERIENCE_ORB_PICKUP;
                    mc.getSoundManager().play(net.minecraft.client.resources.sounds.SimpleSoundInstance.forUI(sound, pitch));
                }
            });
        }
    }
