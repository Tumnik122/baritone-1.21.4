...
public final class Settings {
    ...
    /**
     * When true, camera rotations will use humanized smooth easing curves, dynamic angular velocity limiting,
     * natural micro-jitter, and exact Minecraft mouse sensitivity GCD quantization.
     * Prevents Aim, Rotation, InstantSnap, and GCD flags on GrimAC, Polar, Vulcan, Matrix, etc.
     */
    public final Setting<Boolean> humanizedRotations = new Setting<>(true);
     ...
     */
    public final Setting<Double> rotationJitter = new Setting<>(0.08D);

    /**
     * Randomize the exact point targeted on a block face using natural Gaussian distribution
     * rather than aiming exclusively at the exact center (0.5, 0.5, 0.5).
     */
    public final Setting<Boolean> randomizeBlockHitPoints = new Setting<>(true);

    /**
     * Strictly enforce raytrace line-of-sight checks before performing block interactions.
     * Prevents Raytrace, GhostBreak, and FastPlace flags.
     */
    public final Setting<Boolean> strictRaytraceOnly = new Setting<>(true);
     ...
     */
    public final Setting<Boolean> renderFilledBoxes = new Setting<>(true);

    /**
     * Alpha transparency of shader filled boxes (0.0 to 1.0).
     */
    public final Setting<Float> filledBoxAlpha = new Setting<>(0.25F);

    /**
     * Render dynamic animated rainbow gradient for the active path line.
     */
    public final Setting<Boolean> renderRainbowPath = new Setting<>(false);

    /**
     * Render dynamic glowing pulsation effects on goal and target block indicators.
     */
    public final Setting<Boolean> renderBlockGlow = new Setting<>(true);
    ...
    /**
     * Master switch for client-side rendering optimizations and FPS boosts
     */
    public final Setting<Boolean> fpsBoostEnabled = new Setting<>(true);

    /**
     * Skip rendering entities that are outside the camera frustum or occluded
     */
    public final Setting<Boolean> cullEntities = new Setting<>(true);

    /**
     * Skip rendering block entities (chests, signs, banners, etc.) that are outside the view frustum or occluded
     */
    public final Setting<Boolean> cullBlockEntities = new Setting<>(true);

    /**
     * Skip rendering particles outside the camera view frustum to save GPU vertex bandwidth
     */
    public final Setting<Boolean> cullParticles = new Setting<>(true);

    /**
     * Skip rendering clouds and sky passes when deep underground in caves
     */
    public final Setting<Boolean> cullCloudsUnderground = new Setting<>(true);

    /**
     * Maximum distance (in blocks) to render non-essential entities (like armor stands, item frames, small mobs)
     */
    public final Setting<Double> entityRenderDistance = new Setting<>(48.0D);

    /**
     * Maximum distance (in blocks) to render complex block entities
     */
    public final Setting<Double> blockEntityRenderDistance = new Setting<>(40.0D);

    /**
     * Use ultra-fast lookup table and SIMD-friendly approximations for trigonometric functions in rendering and pathing
     */
    public final Setting<Boolean> fastMath = new Setting<>(true);
     ...
     */
    public final Setting<Double> autoEatHealthThreshold = new Setting<>(18.0D);

    /**
     * Stop eating when hunger reaches this level (max 20). Useful to conserve rare food.
     * Set to 20 to always eat to full hunger.
     */
    public final Setting<Integer> autoEatStopThreshold = new Setting<>(20);

    /**
     * Blacklist of items that autoEat will never eat. By default empty (block harmful
     * foods in code). Add items here to prevent autoEat from using them.
     */
    public final Setting<List<net.minecraft.world.item.Item>> autoEatBlacklist = new Setting<>(new java.util.ArrayList<>());

    /**
     * When true, autoEat will prioritize using a Golden Apple when player HP drops
     * below autoEatGoldenAppleThreshold, regardless of hunger level.
     */
    public final Setting<Boolean> autoEatGoldenApple = new Setting<>(true);

    /**
     * HP threshold (half-hearts * 2, so 8.0 = 4 hearts) below which autoEat will
     * search for and eat a Golden Apple or Enchanted Golden Apple.
     */
    public final Setting<Double> autoEatGoldenAppleThreshold = new Setting<>(8.0D);

    /**
     * Minimum ticks to wait between finishing one eating sequence and starting the next.
     * Prevents micro-loops. Default: 20 ticks (1 second).
     */
    public final Setting<Integer> autoEatCooldownTicks = new Setting<>(20);

    /**
     * When true, autoEat will not pause pathing to eat if a hostile mob is detected
     * within autoEatCombatRadius blocks. It will still eat when HP is critically low.
     */
    public final Setting<Boolean> autoEatPauseInCombat = new Setting<>(false);

    /**
     * Radius in blocks to check for hostile mobs when autoEatPauseInCombat is enabled.
     */
    public final Setting<Double> autoEatCombatRadius = new Setting<>(8.0D);

    /**
     * When true, autoEat will also search the main inventory (slots 9-35) for food,
     * not just the hotbar. Requires allowInventory=true to move food to hotbar.
     */
    ...
    /**
     * Apply smooth analog-like acceleration and natural strafing curves to player movement inputs
     */
    public final Setting<Boolean> naturalMovement = new Setting<>(true);

    /**
     * Play subtle sound effects when paths complete or goals are reached
     */
    public final Setting<Boolean> soundNotifications = new Setting<>(true);

    /**
     * Display in-game toasts on path completion or critical alerts
     */
    public final Setting<Boolean> toastNotifications = new Setting<>(true);

    /**
     * Screen corner position for Baritone's HUD overlay: "TOP_LEFT", "TOP_RIGHT", "BOTTOM_LEFT", "BOTTOM_RIGHT"
     */
    public final Setting<String> hudPosition = new Setting<>("TOP_LEFT");

    /**
     * Display real-time movement speedometer on the HUD
     */
    public final Setting<Boolean> hudShowSpeed = new Setting<>(true);

    /**
     * Display A* search rate (nodes explored/sec) on the HUD
     */
    public final Setting<Boolean> hudShowAStar = new Setting<>(true);

    /**
     * Display mined/gathered block counters and session rate on the HUD
     */
    public final Setting<Boolean> hudShowMined = new Setting<>(true);

    /**
     * Display FPS optimizer culling stats on the HUD
     */
    public final Setting<Boolean> hudShowFpsBoost = new Setting<>(true);
